...
### Bajo este modelo, podemos especular que una correlación positiva entre gastos de litigio y ganancias se debe a las mayores ganancias de las empresas que más litigan (las del sector tecnológico).
...
Bajo el modelo, un aumento del costo en litigio de un millón de dólares reduce las ganancias en `r round(as.numeric(corr_resids$coefficients[2])*1000, digits = 2)*-1` mil.
...
.pull-right[
### El teorema Frisch-Waugh-Lovell prueba que el resultado de ese procedimiento es análogo a correr una sola regresión que "controla" por la variable que queremos residualizar (En este caso, Tech).
...
Efecto causal estimado: 
Bajo el modelo, un aumento del costo en litigio de un millón de dólares reduce las ganancias en `r round(as.numeric(corr_control$coefficients[2])*1000, digits = 2)*-1` mil.
