...
### Los $\beta_k$ se calculan para minimizar $\sum_{i=1}^N\epsilon_i$ en la data de entrenamiento.
...
- ### 4) Seleccionar nivel de complejidad que minimiza error en data de evaluación.
...
### Cuando corremos un MCO, estamos calculando $\beta_k$s que minimizan $\sum_{i=1}^N\epsilon_i$.

### Ridge regression: Calcula los $\beta_k$s para minimizar $\sum_{i=1}^N\epsilon_i+\lambda*\sum_{k=1}^K\beta_k^2$.

### LASSO regression: Calcula los $\beta_k$s para minimizar $\sum_{i=1}^N\epsilon_i+\lambda*\sum_{k=1}^K|\beta_k|$.
